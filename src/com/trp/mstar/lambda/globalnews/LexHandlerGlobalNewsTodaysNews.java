package com.trp.mstar.lambda.globalnews;
...
public class LexHandlerGlobalNewsTodaysNews implements RequestHandler<LexEvent,Object>{
	...
	public Object handleRequest(LexEvent input, Context context) {
		...
		for(int i=0;i<er.getHeadlines().size();i++){
			sb.append(er.getHeadlines().get(i).getTitle()).append("-").append(er.getHeadlines().get(i).getUrl()).append(", ");
		}
		
			
		DialogAction dialogAction = new DialogAction("Close", "Fulfilled", new Message("PlainText","Response from Lambda slot1 are "+sb.toString()));
